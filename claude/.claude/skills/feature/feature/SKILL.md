...
1. **Write a failing test.** Commit it alone, message noting it's
   expected to fail (e.g. `test: <behavior> (failing)`).
2. **Stop. Let the user run it themselves** and confirm it fails as
   expected. Do not run the test suite on their behalf at this gate —
   wait for their go-ahead.
3. **Implement** the solution for that unit.
4. **Stop. Let the user run the tests and review the change** before
   anything is committed. Do not commit until they say so.
